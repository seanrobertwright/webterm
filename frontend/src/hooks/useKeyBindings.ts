...
export function useKeyBindings(options: UseKeyBindingsOptions = {}): UseKeyBindingsReturn {
  ...
  const enterPrefixMode = useCallback(() => {
    ...
    setPrefixMode({ active: true, timestamp: Date.now() });
  ...
  const handleKeyEvent = useCallback((event: KeyboardEvent): boolean => {
    ...
    return true;
  }, [enabled, prefixKey, prefixMode.active, storeLoaded, prefixBindings, rootBindings, legacyBindings, enterPrefixMode, exitPrefixMode]);
