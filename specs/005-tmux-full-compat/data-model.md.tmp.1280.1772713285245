...
## Frontend Multi-Session State Changes (Added 2026-03-05)

No new entities. This section documents changes to frontend state management to fix session creation/switching/deletion.

### SessionStore — New Action: `resetForSessionSwitch()`

Clears: `currentSession`, `windows`, `activeWindowId`, `error`
Preserves: `savedSessions`, `isLoading`

### PaneStore — New Action: `resetForSessionSwitch()`

Clears: `layout`, `panes`, `activePane`, `zoomedPane`

### WebSocketClient — New Method: `switchSession(newSessionId: string)`

Calls `disconnect()` then `connect(newSessionId)`. No new state fields.

### WebSocketClient — Modified Method: `connect(sessionId?: string)`

Early-return guard modified to allow reconnection when `sessionId` differs from `this.sessionId`.

### State Flow: Session Switch

```
[Connected to Session A]
  → User clicks "New Session" or "Restore Session C"
  → resetForSessionSwitch() on both stores (synchronous)
  → switchSession(newId) — disconnect WS, reconnect with ?sessionId=newId
  → Backend sends connected + windowCreated + paneCreated messages
  → Stores repopulated from incoming messages
  → [Connected to new session]
```

### Validation: Active Session Deletion Guard

- SessionPanel disables delete button when `session.id === currentSession.id`
- Only inactive sessions can be deleted from the panel
